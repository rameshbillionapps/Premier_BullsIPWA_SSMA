	...
	ssp.webdb.db.transaction(function (tx) {
		...
		if (window.localStorage.getItem("ssp_custsalesbubble")==1) {
			if (typeof recbegin == 'undefined') {
				tx.executeSql('SELECT tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.CITY,tblCustomers.STATE,tblCustomers.TECH_NO,tblCustomers.FAV,COUNT(distinct tblSales.SIORNM) AS NUMSALES FROM tblCustomers LEFT JOIN tblSales ON tblCustomers.ACCT_NO = tblSales.SIACT WHERE tblCustomers.ACCT_NO LIKE "%' + keywords + '%" OR tblCustomers.NAME LIKE "%' + keywords + '%" ' + addrwhere + ' GROUP BY tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.TECH_NO,tblCustomers.FAV ORDER BY ' + ((isNaN(keywords)||keywords.length==0)?'tblCustomers.NAME':'tblCustomers.ACCT_NO'), [], loadCustomers, ssp.webdb.onError);
			} else {
			    tx.executeSql('SELECT tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.CITY,tblCustomers.STATE,tblCustomers.TECH_NO,tblCustomers.FAV,COUNT(distinct tblSales.SIORNM) AS NUMSALES FROM tblCustomers LEFT JOIN tblSales ON tblCustomers.ACCT_NO = tblSales.SIACT WHERE tblCustomers.ACCT_NO LIKE "%' + keywords + '%" OR tblCustomers.NAME LIKE "%' + keywords + '%" ' + addrwhere + ' GROUP BY tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.TECH_NO,tblCustomers.FAV ORDER BY ' + ((isNaN(keywords) || keywords.length == 0) ? 'tblCustomers.NAME' : 'tblCustomers.ACCT_NO') + ' LIMIT ' + recbegin + ',' + CUSTPAGING, [], loadCustomers, ssp.webdb.onError);
			}
		} else {
			if (typeof recbegin == 'undefined') {
			    ...
			    tx.executeSql('SELECT tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.CITY,tblCustomers.STATE,tblCustomers.TECH_NO,tblCustomers.FAV FROM tblCustomers WHERE tblCustomers.ACCT_NO LIKE "%' + keywords + '%" OR tblCustomers.NAME LIKE "%' + keywords + '%" ' + addrwhere + ' ORDER BY ' + ((isNaN(keywords) || keywords.length == 0) ? 'tblCustomers.NAME' : 'tblCustomers.ACCT_NO') + ' LIMIT ' + recbegin + ',' + CUSTPAGING, [], loadCustomers, ssp.webdb.onError);
			}
		}	
...
ssp.webdb.getCustomersFav = function () {
    ssp.webdb.db.transaction(function (tx) {
    	loadCustomersContentBlock ();
    	if (window.localStorage.getItem("ssp_custsalesbubble")==1) {
    	    tx.executeSql('SELECT tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.CITY,tblCustomers.STATE,tblCustomers.TECH_NO,tblCustomers.FAV,COUNT(distinct tblSales.SIORNM) AS NUMSALES FROM tblCustomers LEFT JOIN tblSales ON tblCustomers.ACCT_NO = tblSales.SIACT WHERE FAV=1 GROUP BY tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.TECH_NO,tblCustomers.FAV ORDER BY FAV DESC,NAME ', [], loadCustomers, ssp.webdb.onError);
    	} else {
    	    tx.executeSql('SELECT tblCustomers.ACCT_NO,tblCustomers.NAME,tblCustomers.ADDR2,tblCustomers.CITY,tblCustomers.STATE,tblCustomers.TECH_NO,tblCustomers.FAV FROM tblCustomers WHERE FAV=1 ORDER BY FAV DESC,NAME ', [], loadCustomers, ssp.webdb.onError);
    	}   
    });
}
...
ssp.webdb.getCustomersOrders = function (PKIDCUSTOMER) {
    ssp.webdb.db.transaction(function (tx) {
        ...
        tx.executeSql('SELECT COUNT(*) AS ORDERCOUNTITEM, SUM(SIPRC*SIQTY) AS ORDERTOTAL, SUM(SIARM) AS ARMTOTAL, a.SIORNM,a.SIINVO,a.SIDATO,SUM(MOD) AS MODIFIED,SUM(NITROSOLD) AS NITROCOUNT,SUM(SIPOA) AS POATOTAL  FROM tblSales a LEFT OUTER JOIN (SELECT SIORNM, SUM(SIQTY) AS nitrosold FROM tblSales WHERE SICOD = "N2Fill" GROUP BY SIORNM ) nitro ON a.SIORNM  = nitro.SIORNM WHERE SIACT = "' + PKIDCUSTOMER + '" GROUP BY a.SIORNM ORDER BY a.SIDATO DESC, a.SIORNM DESC ', [], loadCustomerOrders, ssp.webdb.onError);
    });
}

ssp.webdb.getCustomersNotes = function (PKIDCUSTOMER) {
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('SELECT * FROM tblCustomerNotes WHERE ACCT_NO = "' + PKIDCUSTOMER  + '" ORDER BY MODSTAMP DESC', [], loadCustomerNotes, ssp.webdb.onError);
    });
}

ssp.webdb.getCustomersAR = function (PKIDCUSTOMER) {
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('SELECT * FROM tblCustomersAR WHERE ACCT_NO = "' + PKIDCUSTOMER  + '"', [], loadCustomerAR, ssp.webdb.onError);
    });
}

ssp.webdb.getCustomerByID = function (ACCOUNT) {
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('SELECT tblCustomers.*, COUNT(distinct tblCustomerNotes.PKIDDroid) AS NUMNOTES  FROM tblCustomers LEFT JOIN tblCustomerNotes ON tblCustomers.ACCT_NO = tblCustomerNotes.ACCT_NO WHERE tblCustomers.ACCT_NO = "' + ACCOUNT + '"', [], loadCustomerDetail, ssp.webdb.onError);
    });
}
...
ssp.webdb.addCustomerNote = function (ACCOUNT) {
    ssp.webdb.db.transaction(function (tx) {
        var currenttime = Math.round(new Date().getTime() / 1000);
         tx.executeSql('INSERT INTO tblCustomerNotes (PKIDDroid, TECH_NO,ACCT_NO,NOTE,MODSTAMP,MOD) values (?,?,?,?,?,?)', [currenttime + "." + window.localStorage.getItem("ssp_TechID"),window.localStorage.getItem("ssp_TechID"), ACCOUNT, $("#txtAddNote").val(), currenttime,1], ssp.webdb.getCustomersNotes(ACCOUNT), ssp.webdb.onError);
    });
	return false;
}
...
ssp.webdb.deleteCustomerNote = function (PKID, DelID) {
    if ($("#delnote" + DelID).attr("class") == "button red") {
        ssp.webdb.db.transaction(function (tx) {
            tx.executeSql("DELETE FROM tblCustomerNotes WHERE PKIDDroid = ?", [PKID], ssp.webdb.getCustomersNotes($('#acctid').html()), ssp.webdb.onError);
        });
    } else {
...
ssp.webdb.deleteCustomer = function (PKID) {
    ...
    if ($("#del" + PKID).attr("class") == "button red") {
        ssp.webdb.db.transaction(function (tx) {
           tx.executeSql('DELETE FROM tblCustomers WHERE ACCT_NO = ? ', [PKID], ssp.webdb.getCustomers(), ssp.webdb.onError);
        });    	
    } else {
...
ssp.webdb.setCustomerFav = function (ACCOUNT) {
    ...
    if ( fav == "img/star.png") {
        ssp.webdb.db.transaction(function (tx) {
            tx.executeSql('UPDATE tblCustomers SET FAV = 0,MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + ACCOUNT + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
        });
        $("#fav_" + ACCOUNT).attr("src", "img/star-empty.png");
    }
    else{
        ssp.webdb.db.transaction(function (tx) {
            tx.executeSql('UPDATE tblCustomers SET FAV = 1,MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + ACCOUNT + '"', [], ssp.webdb.onSuccess, ssp.webdb.onError);
        });
...
ssp.webdb.setCustomerCount = function () {
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('SELECT COUNT(*) AS cntCustomer FROM tblCustomers', [],
            function (tx, rs) {
                if (rs.rows.length > 0) {
...
ssp.webdb.SaveCustomerBusiness = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET SP1 = "' + businessaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerContact = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET SP2 = "' + contactaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerPhone = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET ADDR1 = "' + phoneaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerStreet = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET ADDR2 = "' + streetaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerCity = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET CITY = "' + cityaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerState = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET STATE = "' + stateaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerZip = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET ZIP = "' + zipaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerEmail = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET MEMBER = "' + emailaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerHerd = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET SP5 = "' + herdaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerBreed1 = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET BREED1 = "' + breed1addr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerBreed2 = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET BREED2 = "' + breed2addr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerBreed3 = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET BREED3 = "' + breed3addr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerRoute = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET SP4 = "' + routeaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerNote = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET SP6 = "' + noteaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
ssp.webdb.SaveCustomerNitroDays = function () {
    ...
    ssp.webdb.db.transaction(function (tx) {
        tx.executeSql('UPDATE tblCustomers SET SP7 = "' + nitrodaysaddr + '",MOD=1,MODSTAMP=' + currenttime + ' WHERE ACCT_NO = "' + $('#acctid').html() + '"', [], ssp.webdb.onSucess, ssp.webdb.onError);
    });
...
function loadCustomersContentBlock () {
    ...
    rowOutput += '<h1><a href="javascript:void(0)" class="float-right" onclick="loadCustomerNew();"><img src="img/plus-circle-blue.png" width="16" height="16"></a>Customer List<div id="customerkeyword"></div></h1>';
...
function loadCustomers(tx, rs) {
		...
		if (rs.rows.length == 0) {
			...
			for (var i = 0; i < Math.min(CUSTPAGING,rs.rows.length); i++) {
						rowOutput += '<li><a href="#" onclick="ssp.webdb.getCustomerByID(' + "'" + rs.rows.item(i).ACCT_NO + "'" + ');">';
						rowOutput += '<span class="icon"></span>';
						rowOutput += rs.rows.item(i).NAME + '<br>';
						rowOutput += '<small>' + rs.rows.item(i).ADDR2 + '</small>';
						rowOutput += '<br><small>' + rs.rows.item(i).CITY + ', ' + rs.rows.item(i).STATE + '</small>';
						rowOutput += '<div><small><strong>' + rs.rows.item(i).ACCT_NO + '</strong></small></div>';
						rowOutput += '</a>';
						if (rs.rows.item(i).TECH_NO==currtechid) {
							rowOutput += '<ul class="extended-options"><li>';
							rowOutput += (rs.rows.item(i).FAV == 1 ? ('<img id="fav_' + rs.rows.item(i).ACCT_NO + '" width="16" height="16" alt="" src="img/star.png" onclick="ssp.webdb.setCustomerFav(' + "'" + rs.rows.item(i).ACCT_NO + "'" + ')"/>') : ('<img id="fav_' + rs.rows.item(i).ACCT_NO + '" width="16" height="16" alt="" src="img/star-empty.png" onclick="ssp.webdb.setCustomerFav(' + "'" + rs.rows.item(i).ACCT_NO + "'" + ')"/>')); // + '</span>';
							if (window.localStorage.getItem("ssp_custsalesbubble")==1) rowOutput += '<span class="number">' + rs.rows.item(i).NUMSALES + '</span>';
							rowOutput += '</li></ul>';
						} else {
							rowOutput += '<ul class="extended-options"><li><strong>' + rs.rows.item(i).TECH_NO + '</strong></li></ul>';
			...
			var moreOutput = '<div id="customermore"><img src="img/arrow-curve-000-left.png" width="16" height="16" class="picto"> <div style="display:inline-block" id="customerlistcount">' + reccount + '</div> of <div style="display:inline-block" id="customerlisttotal">' + rectotal + '</div> items  ';
...
function loadCustomerDetail(tx, rs) {
	...
	rowOutput += '<h1>';
	rowOutput += '<div id="custname">' + rs.rows.item(0).NAME + '</div><div id="acctid">' + rs.rows.item(0).ACCT_NO + '</div>';
	rowOutput += '<div id="custdisc1" style="display:none">' + rs.rows.item(0).DISC1 + '</div><div id="custdisc2" style="display:none">' + rs.rows.item(0).DISC2 + '</div>';
	if ((window.localStorage.getItem("ssp_projectid") == 'ssc') || (window.localStorage.getItem("ssp_projectid") == 'sess') || (window.localStorage.getItem("ssp_projectid") == 'ecss')) {
	    rowOutput += '<div id="custrate1" style="display:none">' + (rs.rows.item(0).TAXABLE.split(':')[0] * .00001) + '</div><div id="custrate2" style="display:none">' + (rs.rows.item(0).TAXABLE.split(':')[1] * .00001) + '</div><div id="taxstate" style="display:none">' + (rs.rows.item(0).STATE) + '</div>';
	}
    if (window.localStorage.getItem("ssp_projectid") == 'mnss' && rs.rows.item(0).STATE == 'ND' || window.localStorage.getItem("ssp_projectid") == 'mnss' && rs.rows.item(0).STATE == 'MN') {
	    rowOutput += '<div id="custrate1" style="display:none">' + (rs.rows.item(0).TAXABLE.split(':')[0]) + '</div><div id="custrate2" style="display:none">' + (rs.rows.item(0).TAXABLE.split(':')[1]) + '</div><div id="taxstate" style="display:none">' + (rs.rows.item(0).STATE) + '</div>';
	}
	rowOutput += '</h1>';
	...
	rowOutput += '<li><a href="#tab-notes" title="Notes">Notes' + ((rs.rows.item(0).NUMNOTES>0) ? '<span class="nb-events"> ' + rs.rows.item(0).NUMNOTES + '</span>' : '') + '</a></li>';
    ...
    rowOutput += '<li><strong>CONTACT NAME:</strong> ' + rs.rows.item(0).SP2 + '</li>';
    if (window.localStorage.getItem("ssp_projectid") == 'ecss') {       /*JKS051718***v4.01->Added Phone Number edit for ECSS only*/
        rowOutput += '<li>';
        rowOutput += '<strong>PHONE NUMBER:</strong> <div id="CustomerPhone" style="display:inline-block"> ' + rs.rows.item(0).ADDR1 + '&nbsp;&nbsp;<a href="javascript:void(0)" class="float-right" title="modcustphone" onclick="ssp.webdb.modCustomerPhone(\'' + rs.rows.item(0).ADDR1 + '\')"><img src="img/pencil.png" width="16" height="16"/></a></div>';
        rowOutput += '</li>';
    } else {
        rowOutput += '<li><strong>PHONE NUMBER:</strong> ' + rs.rows.item(0).ADDR1 + '</li>';
    }
	rowOutput += '<li><strong>ADDRESS:</strong> ' + rs.rows.item(0).ADDR2 + '</li>';
    ...
    rowOutput += '<li>';
    rowOutput += '<strong>EMAIL:</strong> <div id="CustomerEmail" style="display:inline-block"> ' + rs.rows.item(0).MEMBER + '&nbsp;&nbsp;<a href="javascript:void(0)" class="float-right" title="modcustemail" onclick="ssp.webdb.modCustomerEmail(\'' + rs.rows.item(0).MEMBER + '\')"><img src="img/pencil.png" width="16" height="16"/></a></div>';
    rowOutput += '</li>';
    if (window.localStorage.getItem("ssp_techfunc") == 0) {   //JKS111517***Added IfElse statement to ONLY allow for TechIdMaster to Edit.***
        if (rs.rows.item(0).SP6 == null) {      //JKS122117***3.100.36->Added a check for NULL CustomerNotes***
            rowOutput += '<li>';
            ...
            rowOutput += '<li>';
            rowOutput += '<strong>CUSTOMER NOTE: </strong> <div id="CustomerNote" style="display:inline-block">' + rs.rows.item(0).SP6 + '&nbsp;&nbsp;<a href="javascript:void(0)" class="float-right" title="modcustnote" onclick="ssp.webdb.modCustomerNote(\'' + rs.rows.item(0).SP6 + '\')"><img src="img/pencil.png" width="16" height="16"/></a></div>';
            rowOutput += '</li>';
        }        
    } else {
        if (rs.rows.item(0).SP6 == null) {      //JKS122117***3.100.36->Added a check for NULL CustomerNotes***
            ...
            rowOutput += '<li><strong>CUSTOMER NOTE: </strong>' + rs.rows.item(0).SP6 + '</li>';
        }        
    }
    if (window.localStorage.getItem("ssp_techfunc") == 0) {   //JKS111517***Added IfElse statement to ONLY allow for TechIdMaster to Edit.***
        if ((rs.rows.item(0).SP7 == null) || (rs.rows.item(0).SP7 == " ") || (rs.rows.item(0).SP7 == "")) {      //JKS020718***3.100.37->ADDED both "" and " " to NitroDays check***//JKS013018***3.100.37->Changed or '' to or " " for NitroDays check***//JKS011818***3.100.37->Added or '' to NitroDays check***//JKS122117***3.100.36->Added a check for NULL NitroDays***
            rowOutput += '<li>';
            rowOutput += '<strong>NITROGEN DAYS: </strong> <div id="CustomerNitroDays" style="display:inline-block">' + "" + '&nbsp;&nbsp;<a href="javascript:void(0)" class="float-right" title="modcustnitrodays" onclick="ssp.webdb.modCustomerNitroDays(\'' + rs.rows.item(0).SP7 + '\')"><img src="img/pencil.png" width="16" height="16"/></a></div>';      //JKS021218***3.100.37->Changed Displayed value from window.localStorage.getItem("ssp_nitrodays") to ""***  //JKS020118***3.100.37->Changed " " to window.localStorage.getItem("ssp_nitrodays") for Display***
            rowOutput += '</li>';
        } else {
        rowOutput += '<li>';
        rowOutput += '<strong>NITROGEN DAYS: </strong> <div id="CustomerNitroDays" style="display:inline-block">' + rs.rows.item(0).SP7 + '&nbsp;&nbsp;<a href="javascript:void(0)" class="float-right" title="modcustnitrodays" onclick="ssp.webdb.modCustomerNitroDays(\'' + rs.rows.item(0).SP7 + '\')"><img src="img/pencil.png" width="16" height="16"/></a></div>';
        rowOutput += '</li>';
        }
    } else {
        if ((rs.rows.item(0).SP7 == null) || (rs.rows.item(0).SP7 == " ") || (rs.rows.item(0).SP7 == "")) {      //JKS020718***3.100.37->ADDED both "" and " " to NitroDays check***//JKS013018***3.100.37->Changed or '' to or " " for NitroDays check***//JKS011818***3.100.37->Added or '' to NitroDays check***//JKS122117***3.100.36->Added a check for NULL NitroDays***
            ...
            rowOutput += '<li><strong>NITROGEN DAYS: </strong>' + rs.rows.item(0).SP7 + '</li>';
        }        
    }
	if (rs.rows.item(0).DISC1 == null) {
	    rowOutput += '<li><strong>DISCOUNT %:</strong> 0 </li>';
	}
	else {
	    rowOutput += '<li><strong>DISCOUNT %:</strong> ' + rs.rows.item(0).DISC1 + '</li>';
	}
	rowOutput += '<li><strong>DISC MAX $: </strong> ' + (((!rs.rows.item(0).DISC2)) ? 0 : rs.rows.item(0).DISC2.toFixed(2)); + '</li>';
	if (window.localStorage.getItem("ssp_projectid") == 'ssc') {
	    rowOutput += '<li><strong>G.S.T./H.S.T. (RT869132142):</strong> ' + (rs.rows.item(0).TAXABLE.split(':')[0] * .00001) + '</li>';
	    rowOutput += '<li><strong>T.V.Q. (1087751925):</strong> ' + (rs.rows.item(0).TAXABLE.split(':')[1] * .00001) + '</li>';
	}
	if (window.localStorage.getItem("ssp_projectid") == 'sess') {
	    rowOutput += '<li><strong>Tax Rate 1:</strong> ' + (rs.rows.item(0).TAXABLE.split(':')[0] * .00001) + '</li>';
	    rowOutput += '<li><strong>Tax Rate 2:</strong> ' + (rs.rows.item(0).TAXABLE.split(':')[1] * .00001) + '</li>';
	}
	if (rs.rows.item(0).MOD) {
        rowOutput += '<li><div class="align-right"><a href="javascript:void(0)" class="button" id="del' + rs.rows.item(0).ACCT_NO + '" title="delete" onclick="ssp.webdb.deleteCustomer(' + "'" + rs.rows.item(0).ACCT_NO + "'" + ')"><img src="img/bin.png" width="16" height="16"></a></div></li>';		
	}
	rowOutput += '</ul>';
	rowOutput += '</div>';
    ...
    rowOutput += '<iframe src="' + docUrl + '/Docs/Docs.aspx?type=customer&pkid=' + rs.rows.item(0).ACCT_NO + '" onload="resize_iframe()" frameborder="0" style="overflow:hidden;height:100%;width:100%" height="100%" width="100%"></iframe>';
    //rowOutput += '</body>';
    rowOutput += '</div>';
    rowOutput += '</div>';
	...
	if (window.localStorage.getItem("ssp_projectid") == 'sess') {
	    ...
	    var fcPending = 0;
	    ssp.webdb.db.transaction(function (tx) {
	        tx.executeSql('SELECT SUM(SIQTY*SIPRC) AS amtSales FROM tblSales WHERE (SIINVO = "0" OR SIINVO IS NULL) AND (SILVL4="1.0" OR SILVL4="1") AND SIACT = "' + rs.rows.item(0).ACCT_NO + '"', [],
                function (tx, rs) {
                    if (rs.rows.item(0).amtSales) {
                        fcPending = rs.rows.item(0).amtSales * .60;     /*JKS080818->4.03***SESS Fall Credit rate changed from .50 to .60*/
                    }
    ...
    //check 90 day balance
    	ssp.webdb.db.transaction(function (tx) {
    	    tx.executeSql('SELECT * FROM tblCustomersAR WHERE ACCT_NO = "' + rs.rows.item(0).ACCT_NO + '"', [],
                function (tx, rs) {
                    if (rs.rows.length > 0) {
...
function loadCustomerAR(tx, rs) {
    ...
    if (window.localStorage.getItem("ssp_projectid") == 'mnss') {
        rowOutput += '<li><strong>BALANCE:</strong> ' + rs.rows.item(0).BALANCE + '</li>';
        rowOutput += '<li><strong>0-30:</strong> ' + rs.rows.item(0).CURRENT + '</li>';
        rowOutput += '<li><strong>31-60:</strong> ' + rs.rows.item(0).OVER_DUE + '</li>';
        rowOutput += '<li><strong>OVER 60:</strong> ' + rs.rows.item(0).OVER_90 + '</li>';
        rowOutput += '<li><strong>LAST PAY DATE:</strong> ' + format120Date(rs.rows.item(0).OVER_120) + '</li>';
        rowOutput += '<li><strong>LAST PAYMENT ($):</strong> ' + rs.rows.item(0).PAYMENTS.toFixed(2) + '</li>';
    } else if (window.localStorage.getItem("ssp_projectid") == 'ssp') {
        rowOutput += '<li><strong>BALANCE:</strong> ' + rs.rows.item(0).BALANCE + '</li>';
        rowOutput += '<li><strong>CURRENT:</strong> ' + rs.rows.item(0).CURRENT + '</li>';
        rowOutput += '<li><strong>OVER DUE:</strong> ' + rs.rows.item(0).OVER_DUE + '</li>';
        rowOutput += '<li><strong>OVER 90:</strong> ' + rs.rows.item(0).OVER_90 + '</li>';
        rowOutput += '<li><strong>LAST PAY DATE:</strong> ' + format120Date(rs.rows.item(0).OVER_120) + '</li>';
        rowOutput += '<li><strong>CURRENT MONTH PAYMENT:</strong> ' + rs.rows.item(0).PAYMENTS.toFixed(2) + '</li>';
    } else {
        rowOutput += '<li><strong>BALANCE:</strong> ' + rs.rows.item(0).BALANCE + '</li>';
        rowOutput += '<li><strong>CURRENT:</strong> ' + rs.rows.item(0).CURRENT + '</li>';
        rowOutput += '<li><strong>OVER DUE:</strong> ' + rs.rows.item(0).OVER_DUE + '</li>';
...
function loadCustomerNotes(tx, rs) {
    ...
    if (rs.rows.length == 0) {
        ...
        for (var i = 0; i < rs.rows.length; i++) {
            ...
            rowOutput += '<li>';
			rowOutput += note_html;
			rowOutput += '<br /><strong>'+rs.rows.item(i).TECH_NO+'</strong><em>-' + note_date + '  </em>';
			if (rs.rows.item(i).MOD) {
			    rowOutput += '<a href="#" class="button" id="delnote' + i + '" title="delete" onclick="ssp.webdb.deleteCustomerNote(' + "'" + rs.rows.item(i).PKIDDroid + "'," + i + ')"><img src="img/bin.png" width="16" height="16"></a>';
			}
			rowOutput += '</li>';
        }
    }
...
function loadCustomerOrders(tx, rs) {
    ...
    if (rs.rows.length == 0) {
        ...
        for (var i = 0; i < rs.rows.length; i++) {
			rowOutput += '<li onclick="ssp.webdb.getOrder(' + "'" + rs.rows.item(i).SIORNM + "'" + ','+ ((rs.rows.item(i).MODIFIED > 0) ? 1 : 0) +')">';
			rowOutput += '<a href="#" class="float-left"><img src="img/status' + ((rs.rows.item(i).MODIFIED > 0) ? '-away' : '')  + '.png" width="16" height="16"> ' + rs.rows.item(i).SIORNM + '<h4>' + ((rs.rows.item(i).MODIFIED > 0) ? ('items to sync: ' + rs.rows.item(i).MODIFIED ) : rs.rows.item(i).SIINVO) + '</h4></a>';
			rowOutput += '<ul class="floating-tags float-right">';
			rowOutput += '<li class="tag-date"> ' + formatDate(rs.rows.item(i).SIDATO)  + '</li>';
			rowOutput += '<li class="tag-money"> ' + rs.rows.item(i).ORDERCOUNTITEM + ' - ' + (rs.rows.item(i).ORDERTOTAL + rs.rows.item(i).ARMTOTAL).toFixed(2) + '</li>';
