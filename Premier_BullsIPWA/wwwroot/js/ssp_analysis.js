...
ssp.webdb.getAnalysis = function() {
  ...
  ssp.webdb.db.transaction(function(tx) {
    ...
    tx.executeSql(initialQuery, [], loadAnalysis, ssp.webdb.onError);
  });
};
...
ssp.webdb.AnalysisRefresh = function() {
  ...
  if (!sspValidateDate(fromDate) || !sspValidateDate(toDate)) {
    ...
    ssp.webdb.db.transaction(function(tx) {
      ...
      tx.executeSql(refreshQuery, [], refreshAnalysis, ssp.webdb.onError);
    });
    return false;
  }
};

function refreshAnalysis(tx, rs) {
  var events = new Array();
  for (var i = 0; i < rs.rows.length; i++) {
    events.push({
      title: rs.rows.item(i).TOTAL.toFixed(0),
      start: formatYYYYMMDDtoMMDDYYYY(rs.rows.item(i).SIDATOYYYYMMDD),
      end: formatYYYYMMDDtoMMDDYYYY(rs.rows.item(i).SIDATOYYYYMMDD),
      //backgroundColor: getColorFromValue(rs.rows.item(i).SITYPS)
      //  ? getColorFromValue(rs.rows.item(i).SITYPS)[0].color
      //  : "",
      //textColor: "black",
  ...
  ssp.webdb.db.transaction(function(tx) {
    ...
    if (currentActiveTab == "customer") {
      listQuery =
        ...
        " GROUP BY SIACT, SIANM ORDER BY LineAmount DESC";
    } else if (currentActiveTab == "bull") {
      listQuery =
        "SELECT SICOD, SINAM, SUM((SIQTY * SIPRC) + CASE WHEN SIARM IS NULL THEN 0 ELSE SIARM END) AS LineAmount FROM tblSales WHERE (SITYPS = 'B' OR SITYPS = 'D') AND " +
        appendQuery1 +
        //appendQuery2 +
        //appendQuery3 +
            "SIDATOYYYYMMDD >= " +
            ...
            " AND SIDATOYYYYMMDD <= " +
            formatMDYtoYMD($("#dtpAnalysisToDate").val()) +
        " GROUP BY SINAM ORDER BY LineAmount DESC";
    ...
    tx.executeSql(
      ...
      function(tx, rs) {
        if (rs.rows.length > 0) {
          ...
          rowOutput += "</tr></thead>";
          rowOutput += '<tfoot><tr>';
          rowOutput += '<td colspan="5"><img src="img/arrow-curve-000-left.png" width="16" height="16" class="picto"> <b>Total:</b><div id="totalsales"></div></td>';
          rowOutput += '</tr></tfoot><tbody>';
          rowOutput += "<tbody>";
          for (var i = 0; i < rs.rows.length; i++) {
              totalSales += rs.rows.item(i).LineAmount;
            rowOutput += "<tr>";
            if (currentActiveTab == "customer") {
              rowOutput += "<td>" + rs.rows.item(i).SIACT + "</td>";
              rowOutput += "<td>" + rs.rows.item(i).SIANM + "</td>";
              rowOutput += "<td>" + rs.rows.item(i).LineAmount.toFixed(2) + "</td>";
            } else if (currentActiveTab == "bull") {
              rowOutput += "<td>" + rs.rows.item(i).SICOD + "</td>";
              rowOutput += "<td>" + rs.rows.item(i).SINAM + "</td>";
              rowOutput += "<td>" + rs.rows.item(i).LineAmount.toFixed(2) + "</td>";
            } else if (currentActiveTab == "type") {
              //if (rs.rows.item(i).SITYPS != "D") {
                rowOutput +=
                    "<td>" +
                    //typeSalesValueDomain[rs.rows.item(i).SITYPS]["title"];
                  getSalesTitleFromValue(rs.rows.item(i).SITYPS)[0]["title"] +
                  //rs.rows.item(i).SITYPS
                  "</td>";
                rowOutput += "<td>" + ((rs.rows.item(i).LineAmount) ? rs.rows.item(i).LineAmount.toFixed(2) : "0.00") + "</td>";
              //}
            }
            rowOutput += "</tr>";
          }
