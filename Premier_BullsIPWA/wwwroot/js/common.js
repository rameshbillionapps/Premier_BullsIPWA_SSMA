...
(function($) {
	...
	$.fn.addTemplateSetup(function() {
		...
		this.find(
			'.tabs.same-height, .side-tabs.same-height, .mini-tabs.same-height, .controls-tabs.same-height'
		).equalizeTabContentHeight();

		// Update tabs
		this.find('.js-tabs').updateTabs();

		// Input switches
		this.find('input[type=radio].switch, input[type=checkbox].switch')
			.hide()
			.after('<span class="switch-replace"></span>')
			.next()
			.click(function() {
				$(this).prev().click();
			})
		...
		this.find('input[type=radio].mini-switch, input[type=checkbox].mini-switch')
			.hide()
			.after('<span class="mini-switch-replace"></span>')
			.next()
			.click(function() {
				$(this).prev().click();
			})
	...
	$(document).ready(function() {
		...
		//rp - handles staying on tab when deleting
	...
	$.fn.equalizeTabContentHeight = function() {
		...
		for (i = this.length - 1; i >= 0; --i) {
			...
			for (g = 0; g < tabContainers.length; ++g) {
				tabContainers[g][0].height(
					maxHeight -
						parseInt(tabContainers[g][0].css('padding-top')) -
						parseInt(tabContainers[g][0].css('padding-bottom')) -
						parseInt(tabContainers[g][0].css('margin-bottom')) -
						tabContainers[g][1]
				);
		...
		this.each(function(i) {
			...
			var finalMessage =
				typeof message == 'object'
					? '<ul class="' + messageClass + '"><li>' + message.join('</li><li>') + '</li></ul>'
	...
	$.fn.toggleFieldsetOpen = function() {
		this.each(function() {
			...
			switch (this.id) {
				case 'btnMileageDisplay':
					ssp.webdb.MileageDisplay();
					break;
				case 'btnTimesheetDisplay':
					ssp.webdb.TimesheetDisplay();
					break;
				case 'btnAnalysisDisplay':
					ssp.webdb.AnalysisDisplay();
					break;
			}
		});
	...
	$.fn.addEffectLayer = function(options) {
		...
		this.each(function(i) {
			...
			var layer = $('<div class="loading-mask"><span>' + settings.message + '</span></div>').insertAfter(
				refElement
			);
	...
	function getNodeRefElement(node) {
		...
		if (node.nodeName.toLowerCase() == 'document' || node.nodeName.toLowerCase() == 'body') {
			...
			while (
				...
				!$.inArray($(refElement.parentNode).css('position'), absPos)
			) {
	...
	$.fn.loadWithEffect = function() {
		...
		args[index] = function(responseText, textStatus, XMLHttpRequest) {
			...
			if (textStatus == 'success' || textStatus == 'notmodified') {
				...
				span.css({ marginTop: currentMarginTop + 'px' }).animate(
					{ opacity: 0, marginTop: marginTop - 40 + 'px' },
					{
						complete: function() {
							layer.fadeAndRemove();
						}
				...
				span
					.addClass('error')
					.html(
						$.fn.loadWithEffect.defaults.errorMessage +
							'<br><a href="#">' +
							$.fn.loadWithEffect.defaults.retry +
							'</a> / <a href="#">' +
							$.fn.loadWithEffect.defaults.cancel +
							'</a>'
					);
